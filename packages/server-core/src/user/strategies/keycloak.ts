...
let pdl_access_token
let pdl_refresh_token
let pdl_access_token_data
...
export class KeycloakStrategy extends CustomOAuthStrategy {
  ...
  async getRedirect(data: any, params: CustomOAuthParams): Promise<string> {
    const redirectHost = config.authentication.callback.keycloak
    ...
    if (data instanceof Error || Object.getPrototypeOf(data) === Error.prototype) {
      ...
      returned = returned.concat(`&pdl_access_token=${pdl_access_token}`)
      returned = returned.concat(`&pdl_refresh_token=${pdl_refresh_token}`)
  ...
  async authenticate(authentication: AuthenticationRequest, originalParams: Params) {
    if (authentication.error) {
      if (authentication.error?.error?.type === 'OAuthException')
        throw new Error('You canceled the Keycloak OAuth login flow')
      else
        throw new Error(
          'There was a problem with the Keycloak OAuth login flow: ' + authentication.error?.error?.message
        )
    }
    pdl_access_token = authentication.access_token
    pdl_refresh_token = authentication.refresh_token
    pdl_access_token_data = decodeJWTToken(pdl_access_token)
    const fetchedProfile = {
      sub: pdl_access_token_data.sub
    }
    authentication['profile'] = fetchedProfile
