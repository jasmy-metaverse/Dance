...
const KeycloakCallbackComponent = (props): JSX.Element => {
  ...
  useEffect(() => {
    ...
    // Save the PDL access token & refresh token in the local storage
    localStorage.setItem('pdl_access_token', search.get('pdl_access_token') as string)
    localStorage.setItem('pdl_refresh_token', search.get('pdl_refresh_token') as string)
    localStorage.setItem('pdl_username_updated', 'false')
